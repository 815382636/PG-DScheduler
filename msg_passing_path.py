...
class Postman(object):
    ...
    def get_msg_path(self, job_dags, num_servers):
        ...
        if job_dags_changed:
            ...
            self.dag_enb_summ_backward_map = get_dag_enb_summ_backward_map(job_dags, num_servers)
            self.node_enb_sum_backward_map = get_node_enb_summ_backward_map(job_dags, num_servers)
            self.running_dag_mat = get_running_dag_mat(job_dags)
            self.job_dags = OrderedSet(job_dags)

        return self.msg_mats, self.msg_masks, \
               self.dag_summ_backward_map, self.dag_enb_summ_backward_map,self.node_enb_sum_backward_map,self.running_dag_mat, \
               job_dags_changed
...
def get_dag_enb_summ_backward_map(job_dags, num_server):
    # compute backward mapping from node idx to dag idx
    total_num_nodes = \
        int(np.sum([job_dag.num_nodes for job_dag in job_dags]))
    total_num = total_num_nodes * num_server
    dag_summ_backward_map = \
        np.zeros([total_num, len(job_dags)])
    ...
    for job_dag in job_dags:
        for node in job_dag.nodes:
            for t in range(num_server):
                dag_summ_backward_map[base + n_idx, j_idx] = 1
                n_idx += 1
        base += n_idx
        n_idx = 0
        j_idx += 1
...
def get_node_enb_summ_backward_map(job_dags, num_server):
    # compute backward mapping from node idx to dag idx
    total_num_nodes = \
        int(np.sum([job_dag.num_nodes for job_dag in job_dags]))
    total_num = total_num_nodes * num_server
    dag_summ_backward_map = \
        np.zeros([total_num, total_num_nodes])
    ...
    for job_dag in job_dags:
        for node in job_dag.nodes:
            for t in range(num_server):
                dag_summ_backward_map[base + n_idx, nn_idx] = 1
                n_idx += 1
            nn_idx+= 1
        base += n_idx
        n_idx = 0
...
def get_unfinished_nodes_job_mat(job_dags):
    mats = np.zeros([len(job_dags), 1])
    job_idx = 0
    for job_dag in job_dags:
        for node in job_dag.nodes:
            if node.occupied is True:
                mats[job_idx][0] += 1
        job_idx += 1
    return mats
