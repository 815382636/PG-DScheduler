...
def expand_act_on_state(state, sub_acts):
    ...
    num_nodes = tf.shape(state)[1]  # maybe num_jobs ?
...
def cond(i, x_len,__, _):
    return tf.less(i, x_len)


def body(i, x_len, merge_job, merge_job_copy):
    index = tf.add(i, 1)
    merge_job_copy = tf.zeros([2, 3], dtype=tf.float32)
    return index, x_len, merge_job, merge_job_copy
